...
export function searchNationalities(query: string): Nationality[] {
  ...
  if (aliasCode) {
    ...
    if (exactMatch) {
      return [
        exactMatch,
        ...NATIONALITIES.filter((n) => n.code !== aliasCode).slice(0, 9),
      ];
    }
  }

  const results = NATIONALITIES.filter(
    (n) =>
      n.code.includes(normalizedQuery) || n.name.includes(normalizedQuery),
  );

  results.sort((a, b) => {
    const aExact =
      a.code === normalizedQuery || a.name === normalizedQuery;
    const bExact =
      b.code === normalizedQuery || b.name === normalizedQuery;
